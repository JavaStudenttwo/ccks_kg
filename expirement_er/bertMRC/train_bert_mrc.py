...
from expirement_er.bertMRC.data_loader.model_config import Config
from expirement_er.bertMRC.data_loader.mrc_data_loader import MRCNERDataLoader
from expirement_er.bertMRC.data_loader.mrc_data_processor import Baidu19Processor, YanbaoProcessor
from expirement_er.bertMRC.layer.optim import AdamW, lr_linear_decay
from expirement_er.bertMRC.model.bert_mrc import BertQueryNER
from expirement_er.bertMRC.data_loader.bert_tokenizer import BertTokenizer4Tagger
from expirement_er.bertMRC.metric.mrc_ner_evaluate import flat_ner_performance, nested_ner_performance
...
def args_parser():
    ...
    # requires parameters 
    parser.add_argument("--config_path", default="config/zh_bert.json",
                        type=str)
    parser.add_argument("--data_dir", default="dataset/minibaidu19",
                        type=str)
    parser.add_argument("--bert_model", default="../../bert-base-chinese",
                        type=str, )
    # parser.add_argument("--bert_config", default="/home/nlplab/litiecheng/pycharmcode/tmp/ccks2020/bert-base-chinese/bert_config.json",
    #                     type=str, )
    ...
    parser.add_argument("--data_sign", type=str, default="baidu19")
...
def load_data(config):
    ...
    if config.data_sign == "baidu19":
        data_processor = Baidu19Processor()
    elif config.data_sign == "yanbao":
        data_processor = YanbaoProcessor()
    else:
...
def train(model, optimizer, sheduler, train_dataloader, dev_dataloader, test_dataloader, config, \
    ...
    for idx in range(int(config.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            input_ids, input_mask, output_mask, segment_ids, start_pos, end_pos, span_pos, ner_cate = batch
        ...
        print("Epoch: %s training finished. Time: %.2fs" % (idx, one_epoch_time))
    ...
    print(test_loss_when_dev_best, test_acc_when_dev_best, test_pre_when_dev_best, test_rec_when_dev_best,
...
def eval_checkpoint(model_object, eval_dataloader, config, \
    ...
    for input_ids, input_mask, output_mask, segment_ids, start_pos, end_pos, span_pos, ner_cate in eval_dataloader:
        ...
        with torch.no_grad():
            tmp_eval_loss = model_object(input_ids, segment_ids, input_mask, start_pos, end_pos, span_pos, output_mask)
