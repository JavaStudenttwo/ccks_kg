...
class DataLoader(object):
    ...
    def preprocess(self, data):
        processed = []
        num_sub_tp = len(self.subj_type2id)
        for d in data:
            # d = self.data[i]
            ...
            if items:
                ...
                for j in items:
                    if j['start_index'] >= len(tokens) or j['end_index']-1 >= len(tokens):
                        print(d)
                        continue
                    s1[j['start_index']] = 1
                    s2[j['end_index']-1] = 1
                    stp = self.subj_type2id[j['entity_type']]
                    ts1[j['start_index']] = stp
                    ts2[j['end_index']-1] = stp

                processed += [(tokens_ids, s1, s2, ts1, ts2)]
        return processed
    ...
    def __getitem__(self, key):
        ...
        S1 = np.array(seq_padding(batch[1]))
        S2 = np.array(seq_padding(batch[2]))
        TS1 = np.array(seq_padding(batch[3]))
        TS2 = np.array(seq_padding(batch[4]))

        return (T, S1, S2, TS1, TS2, orig_idx)
