...
class DataLoader(object):
    def __init__(self, bert_tokenizer,  origin_data, predicate2id, subj_type2id, obj_type2id, batch_size=64, evaluation=True):
    ...
    def preprocess(self, data):
        ...
        for d in data:
            ...
            for sp in d['spo_details']:
                if sp[0] >= len(tokens) or sp[1] >= len(tokens):
                    continue
                if sp[4] >= len(tokens) or sp[5] >= len(tokens):
                    continue
                key = (sp[0], sp[1])
                subj_type[key].append(self.subj_type2id[sp[2]])
                if key not in items:
                    items[key] = []
                items[key].append((sp[4], sp[5], self.predicate2id[sp[3]]))
            ...
            if items:
                ...
                for j in items[(k1, k2)]:
                    ...
                    to1[j[0]] = otp
                    to2[j[1]-1] = otp
                processed += [(tokens_ids, distance_to_subj, [k1], [k2-1], s1, s2, o1, o2, ts1, ts2, to1, to2)]
        return processed
    ...
    def __iter__(self):
        ...
        data = self.preprocess(self.origin_data)
    ...
    def __getitem__(self, key):
        ...
        Distance_to_subj = np.array(seq_padding(batch[1]))
        ...
        return (T, K1, K2, Nearest_O1, Distance_O1, S1, S2, O1, O2, TS1, TS2, TO1, TO2, Nearest_S1, Distance_S1, Distance_to_subj, orig_idx)
